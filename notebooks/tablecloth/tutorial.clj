...
(swap! gen-test/*special-value-representations
       conj {:pred tc/dataset?
             :repr (fn [ds] `(tc/dataset ~(-> ds
                                              (update-vals vec)
                                              (->> (into {})))))})
...
(-> (map int-array [[1 2] [3 4] [5 6]])
    ...
    (tc/dataset {:layout :as-rows
                 :column-names [:a :b]}))"]
     (gen-test/write-tests "tutorial-generated-test"
                           "test/tablecloth/tutorial_generated_test.clj"))
