...
In ML world very often you need to test given model and prepare collection of train and test datasets. `split` creates a lazy sequence of maps containing splitted dataset. Map contains two datasets under `:train` and `:test` keys.
...
`tablecloth.pipeline` exports special versions of API which create functions operating only on dataset. These creates the possibility to chain operations and compose them easily.
...
Custom pipeline operator is just function which returns another function operating on a dataset.
